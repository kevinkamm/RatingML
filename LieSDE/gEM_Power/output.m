...
fprintf(inputFile,...
        '\\paragraph*{TimeGAN}\n');
[latexFilePath,latexCommand]=saveRatingProperties(mDCgan,sDDgan,dMLgan,iRSgan,rSOgan,'GAN','ratingProperties');
fprintf(inputFile,...
        '\t\\input{%s}\n',changeSlash(latexFilePath));
for iLC=1:1:length(latexCommand)
    fprintf(inputFile,...
...
fprintf(inputFile,...
        '\\paragraph*{SDE}\n');
% SDE
[latexFilePath,latexCommand]=saveRatingProperties(mDCcal,sDDcal,dMLcal,iRScal,rSOcal,'SDE','ratingProperties');
fprintf(inputFile,...
        '\t\\input{%s}\n',changeSlash(latexFilePath));
for iLC=1:1:length(latexCommand)
    fprintf(inputFile,...
...
if ~isempty(figRD)
    ...
    latexFilePath=saveFigures(figRD,'RD',['RD']);
    fprintf(inputFile,...
            '\t\\input{%s}\n',changeSlash(latexFilePath));
end
...
if ~isempty(figTra)
    ...
    latexFilePath=saveFigures(figTra,'Tra',['Tra']);
    fprintf(inputFile,...
            '\t\\input{%s}\n',changeSlash(latexFilePath));
end
...
%% Latex functions
    ...
    function [latexFilePath,latexCommand]=saveCalibrationParameters(saveAt)
            ...
            file = fopen([tempPath,'/',latexFilePath],'w+');
            latexCommand{end+1}=['\calibrationParameters'];
            fprintf(file,'\\newcommand{%s}{\n',latexCommand{end});
    ...
    function [latexFilePath,latexCommand]=saveComputationalTimes(saveAt)
        ...
        file = fopen([tempPath,'/',latexFilePath],'w+');
        fprintf(file,...
                '\\newcommand{\\ctimeCal}{%g}\n',ctimeCal);
        fprintf(file,...
                '\\newcommand{%s}{\n',latexCommand{1});
        fprintf(file,...
                '\\begin{compactenum}\n');
        fprintf(file,...
                '\\item Calibration time using %d points for the Euler scheme and %d trajectories: \\ctimeCal seconds.\n',N,M);
        fprintf(file,...
                '\\end{compactenum}\n');
        fprintf(file,...
                '}\n');    
    ...
    function [latexFilePath,latexCommand]=saveErrors(saveAt)
        ...
        file = fopen([tempPath,'/',latexFilePath],'w+');
        fprintf(file,...
                '\\newcommand{\\errorCal}{%g}\n',errCal);
        fprintf(file,...
                '\\newcommand{%s}{\n',latexCommand{1});
        fprintf(file,...
                '\\begin{compactenum}\n');
        fprintf(file,...
                '\\item Calibration error using %d points for the Euler scheme and %d trajectories: \\errorCal.\n',N,M);
        fprintf(file,...
                '\\end{compactenum}\n');
        fprintf(file,...
                '}\n');    
